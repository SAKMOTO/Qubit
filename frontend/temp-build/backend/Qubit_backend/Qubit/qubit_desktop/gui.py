...
from PyQt6.QtGui import QFont, QPalette, QColor, QPixmap, QPainter, QPen, QLinearGradient, QIcon, QDesktopServices
try:
...
class AgentThread(QThread):
    ...
    cloud_url_signal = pyqtSignal(str)
    ...
    def run(self) -> None:
        try:
            for evt in run_task_stream(self.task, self.use_cloud, self.headed, self.channel):
                ...
                elif evt[0] == "cloud_url" and isinstance(evt[1], str):
                    self.cloud_url_signal.emit(evt[1])
                elif evt[0] == "done":
...
class QubitMainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._last_cloud_url: Optional[str] = None
        ...
        # Optional cloud browser preview
        self.cloud_view = None
        if QWebEngineView is not None:
            self.cloud_view = QWebEngineView()
            self.cloud_view.setVisible(False)
            self.cloud_view.setMinimumHeight(280)
            layout.addWidget(self.cloud_view)

        # External open button for live URL
        self.open_live_btn = QPushButton("🔗 Open Live View in Browser")
        self.open_live_btn.setVisible(False)
        self.open_live_btn.setEnabled(False)
        self.open_live_btn.clicked.connect(self.open_live_in_browser)
    ...
    def on_run(self) -> None:
        ...
        if hasattr(self.worker, 'cloud_url_signal'):
            self.worker.cloud_url_signal.connect(self.on_cloud_url)
        self.worker.start()
    ...
    def on_cloud_url(self, url: str) -> None:
        self.append_log(f"☁️ Cloud live view: {url}")
        self._last_cloud_url = url
        self.open_live_btn.setVisible(True)
        self.open_live_btn.setEnabled(True)
        if self.cloud_view is not None:
            try:
                self.cloud_view.setVisible(True)
                self.cloud_view.setUrl(QUrl.fromUserInput(url))
            except Exception:
                pass

    def open_live_in_browser(self) -> None:
        if not self._last_cloud_url:
            return
        try:
            QDesktopServices.openUrl(QUrl.fromUserInput(self._last_cloud_url))
        except Exception:
            import webbrowser
            try:
                webbrowser.open(self._last_cloud_url)
            except Exception:
                pass
